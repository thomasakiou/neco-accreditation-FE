...
export default function HeadOfficeStates() {
    ...
    const handleDeleteAll = async () => {
        if (!window.confirm('Are you sure you want to delete all states? This action cannot be undone.')) return;
    ...
    const handleLockAll = async (lock: boolean) => {
        if (!window.confirm(`Are you sure you want to ${lock ? 'lock' : 'unlock'} all states?`)) return;
        try {
            setIsLoading(true);
            if (lock) {
