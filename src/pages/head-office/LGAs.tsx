...
export default function LGAs() {
    ...
    const handleAddLga = async (e: React.FormEvent) => {
        ...
        try {
            ...
            await DataService.createLGA(newLga);
            setShowAddModal(false);
            setNewLga({ name: '', code: '', state_code: '', status: 'active' });
            fetchInitialData();
        } catch (err: any) {
            setError(err.response?.data?.detail?.[0]?.msg || 'Failed to create LGA.');
        } finally {
            setIsSubmitting(false);
        }
    };

    const handleEditClick = (lga: LGA) => {
        setEditingLga(lga);
        setShowEditModal(true);
    };

    const handleUpdateLga = async (e: React.FormEvent) => {
        e.preventDefault();
        if (!editingLga) return;

        try {
            setIsSubmitting(true);
            await DataService.updateLGA(editingLga.code, editingLga);
            setShowEditModal(false);
            setEditingLga(null);
            fetchInitialData();
        } catch (err: any) {
            setError(err.response?.data?.detail?.[0]?.msg || 'Failed to update LGA.');
        } finally {
            setIsSubmitting(false);
        }
    };

    const handleDeleteLga = async (code: string, name: string) => {
        setConfirmDialog({
            isOpen: true,
            title: 'Delete LGA',
            message: `Are you sure you want to delete "${name}"? All associated custodians and schools will be affected.`,
            confirmLabel: 'Delete',
            variant: 'danger',
            onConfirm: async () => {
                try {
                    setIsDeleting(true);
                    await DataService.deleteLGA(code);
                    fetchInitialData();
                    setConfirmDialog(prev => ({ ...prev, isOpen: false }));
                } catch (err: any) {
                    setError('Failed to delete LGA.');
                } finally {
                    setIsDeleting(false);
                }
            },
    ...
    const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            setIsUploading(true);
            setError(null);
            await DataService.uploadLGAs(file);
            fetchInitialData();
            // Clear the input
            if (fileInputRef.current) fileInputRef.current.value = '';
        } catch (err: any) {
            setError(err.response?.data?.detail?.[0]?.msg || 'Failed to upload LGAs. Please ensure the CSV format is correct.');
        } finally {
    ...
    const filteredLgas = lgas.filter(lga => {
        ...
        const matchesState = !selectedState || lga.state_code === selectedState;
    ...
    const paginatedLgas = filteredLgas.slice(startIndex, startIndex + rowsPerPage);

    // Reset to page 1 when search term, selectedState or rowsPerPage changes
    useEffect(() => {
        setCurrentPage(1);
    }, [searchTerm, selectedState, rowsPerPage]);
        ...
        <>
            <div className="space-y-6 animate-in fade-in duration-500">
                ...
                <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div>
                        <h1 className="text-2xl font-bold text-slate-950 dark:text-white flex items-center gap-3">
                            <LocateFixed className="w-8 h-8 text-emerald-600" />
                            Local Government Areas (LGAs)
                        </h1>
                        <p className="text-slate-700 dark:text-slate-400 mt-1 font-medium">Manage National LGAs and their state associations.</p>
                    </div>
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-300 dark:border-slate-800 w-full max-w-lg overflow-hidden animate-in zoom-in-95">
                            <div className="p-6 border-b border-slate-300 dark:border-slate-800 flex items-center justify-between">
                                <h2 className="text-xl font-bold text-slate-950 dark:text-white">Add New LGA</h2>
                                <button onClick={() => setShowAddModal(false)} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>

                            <form onSubmit={handleAddLga} className="p-6 space-y-4">
                                <div className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-black uppercase text-slate-600 tracking-widest">LGA Name</label>
                                        <input
                                            type="text"
                                            required
                                            placeholder="e.g. Ikeja"
                                            value={newLga.name}
                                            onChange={e => setNewLga({ ...newLga, name: e.target.value })}
                                    ...
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-black uppercase text-slate-400 tracking-widest">LGA Code</label>
                                        <input
                                            type="text"
                                            required
                                            placeholder="e.g. 024"
                                            value={newLga.code}
                                            onChange={e => setNewLga({ ...newLga, code: e.target.value.toUpperCase() })}
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select
                                            required
                                            value={newLga.state_code}
                                            onChange={e => setNewLga({ ...newLga, state_code: e.target.value })}
                    ...
                    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm animate-in fade-in">
                        <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-2xl border border-slate-200 dark:border-slate-800 w-full max-w-lg overflow-hidden animate-in zoom-in-95">
                            <div className="p-6 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
                                <h2 className="text-xl font-bold text-slate-900 dark:text-white">Edit LGA: {editingLga.name}</h2>
                                <button onClick={() => { setShowEditModal(false); setEditingLga(null); }} className="text-slate-400 hover:text-slate-600 dark:hover:text-slate-300">
                                    <X className="w-6 h-6" />
                                </button>
                            </div>

                            <form onSubmit={handleUpdateLga} className="p-6 space-y-4">
                                <div className="space-y-4">
                                    <div className="space-y-1.5">
                                        <label className="text-sm font-black uppercase text-slate-400 tracking-widest">LGA Name</label>
                                        <input
                                            type="text"
                                            required
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select
                                            required
                                            value={editingLga.state_code}
                                            onChange={e => setEditingLga({ ...editingLga, state_code: e.target.value })}
                                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                                        >
                                            <option value="">Select State</option>
                                            {states.map(state => (
                                    ...
                                    <div className="space-y-1.5">
                                        ...
                                        <select
                                            required
                                            value={editingLga.status}
                                            onChange={e => setEditingLga({ ...editingLga, status: e.target.value })}
                                            className="w-full px-4 py-2.5 bg-slate-50 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded-xl focus:ring-2 focus:ring-emerald-500 outline-none transition-all"
                                        >
                                ...
                                <div className="pt-4 flex gap-3">
                                    <button
                                        type="button"
                                        onClick={() => { setShowEditModal(false); setEditingLga(null); }}
                                        className="flex-1 px-4 py-2.5 border border-slate-200 dark:border-slate-700 text-slate-600 dark:text-slate-300 rounded-xl font-bold hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                                    >
                                        Cancel
                                    </button>
                ...
                <div className="bg-white dark:bg-slate-900 rounded-2xl shadow-sm border border-slate-300 dark:border-slate-700 overflow-hidden">
                    <div className="p-4 border-b border-slate-300 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4 bg-slate-200 dark:bg-slate-900/50">
                            ...
                            {isSuperAdmin && (
                                <div className="flex items-center gap-2 ml-auto">
                                    <button
                                        ...
                                        className="flex items-center gap-2 px-3 py-2 bg-white dark:bg-slate-800 border border-slate-300 dark:border-slate-700 rounded-xl text-xs font-bold text-slate-700 dark:text-slate-300 hover:bg-slate-50 dark:hover:bg-slate-800 transition-all shadow-sm disabled:opacity-50"
                                        title="Export Excel"
                                    >
                    ...
                    <div className="overflow-x-auto">
                        <table className="w-full text-left border-collapse">
                            <thead className="bg-slate-200 dark:bg-slate-800/80 text-slate-700 dark:text-slate-400 text-[11px] font-bold uppercase tracking-wider">
                                <tr>
                                    <th className="px-6 py-4">LGA Code</th>
                                    ...
                                    <tr>
                                        <td colSpan={4} className="px-6 py-12 text-center">
                                            <div className="flex flex-col items-center gap-2">
                                                <LocateFixed className="w-12 h-12 text-slate-200 dark:text-slate-700" />
                                                <p className="text-slate-500 dark:text-slate-400 font-medium">No LGAs found</p>
                                            </div>
                                        </td>
                                    </tr>
                                ) : (
                                    paginatedLgas.map((lga) => (
                                        <tr key={lga.code} className="group hover:bg-slate-200/50 dark:hover:bg-slate-800/40 transition-colors">
                                            ...
                                            <td className="px-6 py-4">
                                                <span className="text-sm font-bold text-slate-900 dark:text-slate-300 flex items-center gap-2">
                                                    <div className="w-1.5 h-1.5 rounded-full bg-emerald-500" />
                                                    {states.find(s => s.code === lga.state_code)?.name || lga.state_code}
                                                </span>
                                            </td>
                                            <td className="px-6 py-4 text-right">
                                                <div className="flex items-center justify-end gap-2">
                                                    ...
                                                    <button
                                                        onClick={() => handleDeleteLga(lga.code, lga.name)}
                    ...
                    {!isLoading && filteredLgas.length > 0 && (
                        <div className="p-4 border-t border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row items-center justify-between gap-4 text-sm text-slate-500 dark:text-slate-400">
                            <p>
                                ...
                                <span className="font-semibold text-slate-900 dark:text-white">
                                    {Math.min(startIndex + rowsPerPage, filteredLgas.length)}
                                </span> of{' '}
                                <span className="font-semibold text-slate-900 dark:text-white">{filteredLgas.length}</span> LGAs
                            </p>
