...
export default function ViewerSchools() {
  ...
  React.useEffect(() => {
    const fetchSchoolsAndData = async () => {
      try {
        const [schoolsData, statesData, zonesData, userData] = await Promise.all([
      ...
      <div className="bg-white dark:bg-slate-900 rounded-lg border border-slate-200 dark:border-slate-800 p-6">
        <div className="mb-6 flex flex-col sm:flex-row gap-4">
          ...
          <select
            ...
            className="px-4 py-2 border border-slate-300 dark:border-slate-600 rounded-lg bg-white dark:bg-slate-800 text-slate-950 dark:text-white focus:outline-none focus:ring-2 focus:ring-blue-500 w-full sm:w-48"
          >
          ...
          <label className="flex items-center gap-2 cursor-pointer group">
            <div className="relative">
              <input
                type="checkbox"
                checked={isDueOnly}
                onChange={(e) => setIsDueOnly(e.target.checked)}
                className="peer sr-only"
              />
              <div className="w-5 h-5 rounded border-2 border-slate-300 dark:border-slate-700 peer-checked:border-red-500 peer-checked:bg-red-500 transition-all"></div>
              <div className="absolute inset-0 flex items-center justify-center opacity-0 peer-checked:opacity-100 transition-opacity">
                <AlertCircle className="w-3 h-3 text-white" />
              </div>
            </div>
            <span className="text-sm font-bold text-slate-600 dark:text-slate-400 group-hover:text-red-600 transition-colors uppercase tracking-widest">
              Due Only
            </span>
          </label>
        </div>
                        ...
                        <table className="w-full text-sm">
                          <thead>
                            <tr className="border-b border-slate-200 dark:border-slate-700">
                              ...
                              <th className="text-left py-3 px-4 font-semibold text-slate-950 dark:text-white">Gender</th>
                            ...
                            {stateSchools.map((school) => (
                              <tr key={school.code} className="border-b border-slate-200 dark:border-slate-700 hover:bg-white dark:hover:bg-slate-800 transition-colors">
                                ...
                                <td className="py-3 px-4 text-slate-700 dark:text-slate-300">
                                  {school.name}
                                  {isSchoolDueForAccreditation(school, states, zones) && (
                                    <span className="ml-2 text-red-600 font-bold">(DUE)</span>
                                  )}
                                </td>
                                <td className="py-3 px-4 text-slate-700 dark:text-slate-300">{school.category || '-'}</td>
                                <td className="py-3 px-4 text-slate-700 dark:text-slate-300">{school.gender || '-'}</td>
