...
export default function ViewerZones() {
  ...
  if (isLoading) {
    return <div className="flex items-center justify-center p-8"><div className="text-slate-600 dark:text-slate-400">Loading zones...</div></div>;
  }

  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-3xl font-bold text-slate-950 dark:text-white">Zones</h1>
        <p className="text-slate-600 dark:text-slate-400 mt-2">View all accreditation zones</p>
      </div>
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {filteredZones.map((zone) => (
                <tr key={zone.code} className="border-b border-slate-200 dark:border-slate-700 hover:bg-slate-50 dark:hover:bg-slate-800/50">
                  <td className="py-3 px-4 text-slate-950 dark:text-white font-medium">{zone.code}</td>
                  <td className="py-3 px-4 text-slate-700 dark:text-slate-300">{zone.name}</td>
                </tr>
              ))}
            </tbody>
          </table>
